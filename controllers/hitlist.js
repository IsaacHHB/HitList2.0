...
module.exports = {
    ...
    details: async (req, res) => {
        ...
        try {
            ...
            const hitlistItems = await Hitlist.find({ userId: req.params.userID })
            const itemsLeft = await Hitlist.countDocuments({ userId: req.user.id, applied: false })
            const list = await Hitlist.findById(req.params.id).populate('user').lean()

            if (list) {
                return response.render('error404.ejs')  
              }
           else {
                res.render('details.ejs', { hitlist: hitlistItems, items: list, left: itemsLeft, user: req.user })
              }
    ...
    createHitlist: async (req, res) => {
        try {
            await Hitlist.create({ name: req.body.name, url: req.body.url, position: req.body.position, company: req.body.company, email: req.body.email, typeOfPosition: req.body.typeOfPosition, applied: false, userId: req.user.id })
    ...
    editHitlist: async (req, res) => {
        try {
                const list = await Hitlist.findOne(req.params.id).populate('user').lean()
